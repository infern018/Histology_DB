...
export const deletePrivateCollection = async (dispatch,id,user) =>{
    ...
    try {
        const res = await axios.delete(`${process.env.REACT_APP_API_URL}/collections/${id}/${user._id}`,
        {
            headers:{
...
export const createPrivateCollection = async (dispatch,collection,user) =>{
    ...
    try {
        const res = await axios.post(`${process.env.REACT_APP_API_URL}/collections/${user._id}`,collection,
        {
            headers:{
                'token':`Bearer ${user.accessToken}`
            }
...
export const updatePrivateCollection = async (dispatch,collection,user) =>{
    ...
    try {
        const res = await axios.put(`${process.env.REACT_APP_API_URL}/collections/${collection._id}/${user._id}`,collection,
        {
            headers:{
                'token':`Bearer ${user.accessToken}`
            }
...
export const updateRequests = async (dispatch,collection,user) =>{
    //CHECK USER METHOD FOR THIS (request methods me)
    try {
        const res = await axios.put(`${process.env.REACT_APP_API_URL}/collections/${collection._id}/${user._id}`,{publicStatus:'approved',visibility:'public'},{
            headers: {
                'token': `Bearer ${user.accessToken}`
            }
        })
...
export const deleteRequest = async (dispatch,id,user) =>{
    ...
    try {
        const resp = await axios.put(`${process.env.REACT_APP_API_URL}/collections/${id}/${user._id}`,{publicStatus:'declined',visibility:'private'},{
            headers: {
                'token': `Bearer ${user.accessToken}`
            }
        })
